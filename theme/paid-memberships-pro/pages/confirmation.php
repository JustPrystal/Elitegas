...
// 		$confirmation_message = "<p>" . sprintf(__('Thank you for joining elitegas.com. Your membership is now active and a welcome email has been sent to %s. ', 'paid-memberships-pro' ), $current_user->user_email, get_bloginfo("name"), $current_user->membership_level->name) . "</p>";
	$confirmation_message = "<p>" . sprintf(__('Thank you for joining elitegas.com. Your membership is now active and a welcome email has been sent to %s. ', 'paid-memberships-pro' ), $current_user->user_email, get_bloginfo("name"), $current_user->membership_level->name) . "</p>";
...
<?php if(!empty($pmpro_invoice) && !empty($pmpro_invoice->id)) { ?>
	...
	<?php
		...
		$confirmation_message .= "<p>" . sprintf(__('Below are details about your account and a receipt for your initial membership invoice. A welcome email with a copy of your initial membership invoice has been sent to %s.', 'paid-memberships-pro' ), $pmpro_invoice->user->user_email) . "</p>";
...
<?php
	...
	{
		$confirmation_message .= "<p>" . sprintf(__('Below are details about your account.', 'paid-memberships-pro' ) ) . "</p>";
...
<p>To see our prices and place orders, please fill out the legal information on the next page to apply to become an Elite Gas Distributor.
	</p>
	</p>
<?php
	}
?>
<p class="View-membership-url">
	<?php if ( ! empty( $current_user->membership_level ) ) { ?>
		<a class="View-url" href="https://elitegas.com/my-account/boarding-info/"><?php _e( 'View Your Membership Account &rarr;', 'paid-memberships-pro' ); ?></a>
	<?php } else { ?>

...
startWatcher = async () => {
    ...
    compileSass = async () => {
        ...
        await sass.render({
            file: './src/scss/responsive.scss',
            outFile: './theme/responsive.css',
            sourceMap: true
        }, async (err, result) => {
            if (err) { console.log(err) }
            await fs.writeFile('./theme/responsive.css', result.css, (error) => {
                if (error) { console.log(error) }
            })
            await fs.writeFile('./theme/responsive.css.map', result.map, (error) => {
                if (error) { console.log(error) }
            })
        })
    }
